...
class Session:
    ...
    def get_chat(self, sid, model=None, debug=False):
        if model is None:
        ...
        if self.chat is None:
            ...
            if debug:
                logger.info(
                    f"Session {sid} model changed from {self.chat['model']} to {model}"
                )
            self.clear_chat()
            self.chat = {"engine": chat_tools.ChatEngine(model), "model": model}
...
class SessionManager:
    ...
    def get_chat(self, sid, model=None, debug=False):
        return self.get_session(sid).get_chat(sid, model, debug)


def test_chat_manager():
    chat_manager = SessionManager.get_instance()
    x = chat_manager.get_chat("test", "gemini")
    print(x.predict("Hello"))
    x = chat_manager.get_chat("test", chat_tools.DEFAULT_CHAT_LLM)
    print(x.predict("Hello"))


def get_messages(args): # move to class
    """
...
def remove_session(args): # move to class
    """
    Clear a session
    """
...
def save_message(content, args, detail): # move to class
    """
...
def update_session_name(user_id, sid):
    """
    Check user's all session, if sname is empty, set sname
    """
    ret = True
    items = StoreMessage.objects.filter(
        Q(user_id=user_id) & (Q(sname__isnull=True) | Q(sname=sid))
    ).values("sid").distinct()
    ...
    for item in items:
        ...
        if not set_session_name(user_id, item["sid"]):
            ret = False
    if ret:
...
def set_session_name(user_id, sid):
    items = StoreMessage.objects.filter(
    ...
    if not sname_empty:
        return True
    if sname == "":
        sname = calc_session_name(user_id, items)
    if sname != "":
